...
Add `swift-google-cloud-texttospeech-v1` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-texttospeech-v1.git --from 0.1.0-preview
```

Then add `GoogleCloudTextToSpeechV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudTextToSpeechV1 <target-name> --package swift-google-cloud-texttospeech-v1
```
